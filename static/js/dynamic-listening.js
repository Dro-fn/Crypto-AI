...
analysisButton.addEventListener("click", async () => {
    ...
    if (!selectedCrypto || selectedCrypto === "Select Cryptocurrency") {
        console.error("No cryptocurrency selected. Please choose one.");
        analysisResponseDiv.textContent = "Please select a cryptocurrency to proceed.";
        analysisResponseContainer.style.display = "block";
        return;
    }

    // Clear the analysis box and show a loading indicator
    analysisResponseDiv.innerHTML = "<p>Fetching analysis and recommendation...</p>";
    analysisResponseContainer.style.display = "block";
    ...
    try {
        const response = await fetch(API_BASE_URL + "/chat_recommendation", {
            ...
            body: JSON.stringify({ 
                user_id: 'default', 
                message: "Request for analysis and recommendation", 
                crypto: selectedCrypto // Pass the selected cryptocurrency
            }),
        });
        ...
        analysisResponseDiv.innerHTML = formattedResponse; // Use innerHTML to render <br> tags
        analysisResponseContainer.style.display = "block";
        // Display the response in the analysis response box
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!endButton) {
        ...
        return;  // ✅ Return here only if it's inside a function
    }
